...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/20 19:54:20 by nilamber          #+#    #+#             */
/*   Updated: 2024/05/20 20:07:02 by nilamber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	char		*raisin;
	static char	pepin[1026][BUFFER_SIZE + 1];
	int			rv;

	eau[BUFFER_SIZE] = 0;
	rv = 1;
	if (itn(pepin[fd], ft_strlen(pepin[fd])))
	{
		raisin = son(pepin[fd], pepin[fd], ft_strlen(pepin[fd]));
		return (raisin);
	}
	raisin = NULL;
	if (*pepin[fd])
		raisin = ft_strjoin(pepin[fd], raisin);
	pepin[fd][0] = 0;
	while (!*pepin[fd] && (!raisin || raisin[ft_strlen(raisin) - 1] != '\n')
		&& rv > 0)
	{
		ft_bzero(eau, BUFFER_SIZE + 1);
		rv = read(fd, eau, BUFFER_SIZE);
		raisin = free_son(raisin, eau, pepin[fd], rv);
	}
	return (raisin);
}
